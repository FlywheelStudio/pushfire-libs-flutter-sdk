...
- **Supabase Auth Listener**: Fixed auto-subscriber registration for existing sessions
  - Handle `initialSession` event so users already logged in are registered on SDK init
  - Handle `userUpdated` event to re-register subscriber when profile changes
  - Previously only `signedIn` was handled, missing already-authenticated users
